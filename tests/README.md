...
全站自动化冒烟测试：15 个页面零错误加载 + 移动端布局 + 主题切换 + 错题本重做全流程 e2e + 五子棋/贪吃蛇引擎单测与 e2e。
...
| 检查 | 内容 |
|------|------|
| 全页面加载 | 15 个页面（主页、实验室、7 模块、5 玩具、生日页）零 console error / pageerror / 请求失败 |
...
| 贪吃蛇 e2e | 暂停 → 继续 → 键盘转向 |
| 拾贝文件流程 e2e | 导入文件 → 查看弹窗 → 图片点击放大/关闭 → 删除 → 确认（无残留） |
